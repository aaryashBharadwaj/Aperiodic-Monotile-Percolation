...
from percolation import (
    percolationStatsI,
    percolationStatsU,
    percolationStatsBondI,
    percolationStatsBondU
)
from visualisation import (
    plot_frames,
    plot_percolation_stats_IU,
    plot_extrapolation_IU
)


def trials_for_L(L, default_t):
    if 10 <= L <= 100:
        return 3000
    elif 110 <= L <= 200:
        return 500
...
if __name__ == "__main__":
    ...
    master_nodes, master_neighbors = build_neighbor_graph_fast(
        p, level=args.r + 1
    )
    ...
    for l in l_values:
        t_cur = trials_for_L(l, args.t)
        print(f"\nAnalyzing L={l} with {t_cur} trials")

        fd = analyze_square_frame(
            master_nodes,
            master_neighbors,
            L=l,
        ...
        if fd['node_count'] > 0:
            ...
            if tb_overlap or lr_overlap:
                tqdm.write(
                    f"WARNING L={l}: Boundary overlap! "
                    f"TB={len(tb_overlap)}, LR={len(lr_overlap)}"
                )
        ...
        statsSI = percolationStatsI(
            fd['sub_graph_nodes'],
            fd['sub_graph_neighbors'],
            fd['top_boundary_nodes'],
            fd['bottom_boundary_nodes'],
            fd['left_boundary_nodes'],
            fd['right_boundary_nodes'],
            t_cur
        )
        ...
        mSI.append(statsSI.trials_mean())
        sSI.append(statsSI.trials_std())
        mSU.append(statsSU.trials_mean())
        sSU.append(statsSU.trials_std())
        ...
        mBI.append(statsBI.trials_mean())
        sBI.append(statsBI.trials_std())
        mBU.append(statsBU.trials_mean())
        sBU.append(statsBU.trials_std())

        print(
            f"Site pc Mean: {statsSI.trials_mean():.10f} | "
            f"Bond pc Mean: {statsBI.trials_mean():.10f}"
        )
    ...
    # final plots
    plot_percolation_stats_IU(
        l_values, mSI, sSI, mSU, sSU, mBI, sBI, mBU, sBU
    )
