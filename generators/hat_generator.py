...
# [a,b,c, d,e,f] means (x,y) -> (a*x + b*y + c ,  d*x + e*y + f).

# Point in cartesian space
def pt(x, y):
    return {'x': x, 'y': y}

# Point on the triangular/hex grid
def hexPt(x, y):
    return pt(x + 0.5*y, (sqrt3/2)*y)

# Undoes (inverses) an affine transform. 
def inv(T):
    det = T[0]*T[4] - T[1]*T[3]
    return [T[4]/det, -T[1]/det, (T[1]*T[5]-T[2]*T[4])/det,
            -T[3]/det, T[0]/det, (T[2]*T[3]-T[0]*T[5])/det]

# Compose transforms (result applies B then A)
def mul(A, B):
...
# Applies the standard affine rotation matrix 
# Requires 3 dimensions since 2 by 2 can only make linear transformations in 2D
def trot(ang):
    cos = math.cos(ang)
    sin = math.sin(ang)
    return [cos, -sin, 0, sin, cos, 0]

# Applies the standard affine translation matrix 
# Translation by (tx,ty)
def ttrans(tx, ty):
    return [1, 0, tx, 0, 1, ty]

# Rotation around an arbitrary point p
# First moves p to the centre, then rotates normally, then moves p back
def rotAbout(p, ang):
    return mul(ttrans(p['x'], p['y']), mul(trot(ang), ttrans(-p['x'], -p['y'])))

## Applies a specific transformation matrix to a point P
def transPt(M, P):
    return pt(M[0]*P['x'] + M[1]*P['y'] + M[2], M[3]*P['x'] + M[4]*P['y'] + M[5])

## Creates an affine transformation with similarity so angles are preserved
## The first point is what happens to your centre (0,0) and the second to (1,0)
def matchSeg(p, q):
    return [q['x']-p['x'], p['y']-q['y'], p['x'], q['y']-p['y'], q['x']-p['x'], p['y']]

# Let's say I have a  basis 1 based on (p1, q1) and I draw a picture in it, then 
# Basis 2 (p2, q2) is the same picture but moved and enlarged
# The matchTwo takes any point on the original picture and moves it to the enlarged transformation
# It is the inverse of change of basis, an active transformation

# This is actually the heart of the tiling
# You generate your tile and then transform it based on which vertices it must share
def matchTwo(p1, q1, p2, q2):
    return mul(matchSeg(p2, q2), inv(matchSeg(p1, q1)))

# Finds the intersection point of two lines
# The first line is p1, q1 and the second line is p2 q2
...
# This was all the raw tiling machinery now for the hat specific construction
...
# This is your basic object for the tile
class HatTile:
    def __init__(self):
        self.shape = hat_outline

# Each MetaTile is just a 
class MetaTile:
...
# Each metatile is a construction made using smaller metatiles
# There is an intermediate stage where H, T, P and F tiles are first constructed and combined
# These then form the larger metatile
def H_init():
    H_outline = [
...
# Constructs a larger metatile, a patch, using smaller ones
# ['X'] initialises a tile for any type X amongst H, T, P, F
# [parent, edgeIdx, 'X', k] places X onto the edge spanned between two placed children
...
# Builds the larger level H, T, P, F as new_H, new_T, new_P, new_F
