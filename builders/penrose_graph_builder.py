...
from builders.direct_graph_builder import dedup_vertices   # shared vertex-dedup, common to all DIRECT builders

TOL = 1e-5

# Builds the Penrose (rhomb-lattice) percolation graph using the SAME shared primitives as the
# hat/spectre direct builder: direct_graph_builder.dedup_vertices (one KDTree.query_pairs ndarray +
# scipy connected_components) to merge coincident vertices, then graph_core.edges_to_adjacency.
# Thus validation exercises the exact same pipeline as the hat/spectre production runs.

# Each Penrose rhombus is two Robinson triangles sharing a diagonal; that diagonal is NOT a tiling
# edge, so per triangle we keep only the two OUTER edges (v1-v3 and v1-v2). Across a rhombus's two
# triangles those four outer edges are exactly the rhombus perimeter.
def build_penrose_neighbor_graph(tiling):
    ...
    # Merge coincident vertices into unique node ids (shared graph_core helper).
    ...
    # Assemble neighbour lists with the shared graph_core helper.
